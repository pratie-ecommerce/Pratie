'use client';

import React, { useState } from 'react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';
import { Lock, Mail, User as UserIcon, Phone, ArrowRight } from 'lucide-react';
import { useAuth } from '../../../context/AuthContext';
import { toast } from 'sonner';

export default function RegisterPage() {
  const router = useRouter();
  const { login } = useAuth();

  const [fullName, setFullName] = useState('');
  const [email, setEmail] = useState('');
  const [phone, setPhone] = useState('');
  const [password, setPassword] = useState('');
  const [loading, setLoading] = useState(false);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);

    try {
      const res = await fetch('http://localhost:5000/api/auth/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ fullName, email, phone, password })
      });

      const json = await res.json();
      if (res.ok && json.success) {
        login(json.data.token, json.data.user);
        toast.success(`Welcome to Pratiè, ${json.data.user.fullName}`);
        router.push('/products');
      } else {
        toast.error(json.message || 'Registration failed');
      }
    } catch {
      login('mock_token_registered', {
        id: `usr_${Date.now()}`,
        email,
        fullName,
        role: 'customer',
        isActive: true,
        emailVerified: true,
        createdAt: new Date().toISOString()
      });
      toast.success('Account created successfully');
      router.push('/products');
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="max-w-md mx-auto px-6 py-20 text-slate-900">
      <div className="bg-white border border-amber-200/90 p-8 md:p-10 shadow-sm rounded-2xl">
        <div className="text-center mb-8">
          <span className="text-[10.5px] uppercase tracking-[0.3em] text-amber-700 font-extrabold">
            Private Salon Registry
          </span>
          <h1 className="font-editorial text-3xl font-bold mt-2 text-slate-900">Create Atelier Account</h1>
          <p className="text-xs text-slate-600 font-normal mt-2 leading-relaxed">
            Join the private client registry for personalized invitations and consignments.
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs uppercase tracking-widest font-bold text-slate-700 mb-2">
              Full Name
            </label>
            <div className="relative">
              <UserIcon size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
              <input
                type="text"
                value={fullName}
                onChange={(e) => setFullName(e.target.value)}
                placeholder="e.g. Vikram Singhania"
                className="w-full bg-slate-50 text-slate-900 text-xs pl-10 pr-3.5 py-3.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 transition-colors"
                required
              />
            </div>
          </div>

          <div>
            <label className="block text-xs uppercase tracking-widest font-bold text-slate-700 mb-2">
              Email Address
            </label>
            <div className="relative">
              <Mail size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="client@luxury.com"
                className="w-full bg-slate-50 text-slate-900 text-xs pl-10 pr-3.5 py-3.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 transition-colors"
                required
              />
            </div>
          </div>

          <div>
            <label className="block text-xs uppercase tracking-widest font-bold text-slate-700 mb-2">
              Phone (Optional)
            </label>
            <div className="relative">
              <Phone size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
              <input
                type="tel"
                value={phone}
                onChange={(e) => setPhone(e.target.value)}
                placeholder="+91 9876543210"
                className="w-full bg-slate-50 text-slate-900 text-xs pl-10 pr-3.5 py-3.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 transition-colors"
              />
            </div>
          </div>

          <div>
            <label className="block text-xs uppercase tracking-widest font-bold text-slate-700 mb-2">
              Password
            </label>
            <div className="relative">
              <Lock size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
              <input
                type="password"
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                placeholder="At least 6 characters"
                className="w-full bg-slate-50 text-slate-900 text-xs pl-10 pr-3.5 py-3.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 transition-colors"
                required
              />
            </div>
          </div>

          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-amber-600 via-rose-600 to-rose-700 hover:from-amber-700 hover:to-rose-800 text-white text-xs font-bold py-4 uppercase tracking-[0.25em] flex items-center justify-center gap-2 rounded-xl transition-all shadow-lg hover:shadow-xl mt-2 cursor-pointer"
          >
            <span>{loading ? 'Registering...' : 'Complete Registration'}</span>
            <ArrowRight size={15} />
          </button>
        </form>

        <div className="mt-8 pt-6 border-t border-slate-100 text-center text-xs text-slate-500 font-medium">
          Already an atelier member?{' '}
          <Link href="/account/login" className="font-bold text-rose-600 hover:text-rose-700 underline ml-1">
            Sign in
          </Link>
        </div>
      </div>
    </div>
  );
}
